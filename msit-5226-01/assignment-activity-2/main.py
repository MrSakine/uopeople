...
# 2. Save the downloaded file as 'house_prices.csv' in the same folder as this script.
# ----------------------------------------------------------------------------------
print("--- TASK 1: DATA EXPLORATION AND LOADING ---")

# a. Load the dataset and inspect its structure.
try:
    # We assume the file is named house_prices.csv as per the assignment context
    file_path = 'house_prices.csv'
    df = pd.read_csv(file_path)
    print(f"Dataset successfully loaded from: {os.path.abspath(file_path)}")

except FileNotFoundError:
    print(f"ERROR: File '{file_path}' not found. Please ensure you have downloaded the CSV and saved it in the same directory.")
    # Exiting if file is not found to prevent errors
    exit()
...
# Rename columns for clarity if needed, assuming the columns are 'size' and 'price'
# Adjust these column names if the CSV uses different headers (e.g., 'House Size', 'House Price')
...
# Use plt.savefig('house_price_scatterplot.jpg') if running locally to save the image
plt.show()
